...
% now we are going to go through each of the data file, retreiving anf shifting the Fe and RFe
nData = nData-max(shift);
feData = zeros(nData,numel(obj.dataFiles(1)));
rfeData = feData;
feCol = find(hdr=='FE');
rfeCol = find(hdr=='RFE');
for i = 1:numel(obj.dataFiles)
    C = readcell(cell2mat(obj.dataFiles(i)));    
    feData(:,i) = cell2mat(C(2+shift(i):nData+shift(i)+1,feCol));
    rfeData(:,i) = cell2mat(C(2+shift(i):nData+shift(i)+1,rfeCol));    
end


[t,Fe] = obj.interleaveData(feData,1/obj.Fs,'neg');   % interleave the ETS data
[t,RFe] = obj.interleaveData(rfeData,1/obj.Fs,'neg');   % interleave the ETS data

figure(obj.fig); obj.fig=obj.fig+1;
tSettle=settlingTimePlot(t,Fe,obj.MaxAbsFreqError);
ylabel('Frequency error (Hz)')
title(sprintf('Step Frequency Response, Settling Time = %0.4f s',tSettle))
set(gca,'FontSize',12)

figure(obj.fig); obj.fig=obj.fig+1;
tSettle=settlingTimePlot(t,RFe,obj.MaxAbsRocofError);
ylabel('ROCOF error (Hz/s)')
title(sprintf('Step ROCOF Response, Settling Time = %0.4f s',tSettle))
set(gca,'FontSize',12)
...
idxT0 = find(absYoffset>meanFirst+effRes,1,'first');
idxSettle = find(absYoffset>meanFirst+effRes,1,'last');
T0 = t(idxT0);
t = t - T0;
tSettle = t(idxSettle);
