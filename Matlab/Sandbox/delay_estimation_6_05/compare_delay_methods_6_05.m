% This script compares a number of delay estimation algorithms as 
% signal-to-noise ratio and frequency content vary
%
...
try
    f_cpu=feature('timing', 'cpuspeed');%this may not work on all systems
catch
    f_cpu=3.4e9;%CPU clock rate
    warning('CPU frequency unavailable, using %0.2f GHz',f_cpu*1e-9)
end

method_names={'Parabola','Gaussian','Modified Gaussian','Cosine','Two Point','Phase','Iterative'};
method_functions={'k_d_hat(j,k,m)=delayest_3point(y1n,y0n,''parabola'',''xc'');',...
    ...
    'k_d_hat(j,k,m)=delayest_psarakis(y1n,y0n);',...
    'k_d_hat(j,k,m)=delayest_fft(y1n,y0n);',...
    '[k_d_hat(j,k,m) N_i]=delayest_iterative(y1n,y0n);'};
N_methods=numel(method_names);%number of methods

N_p=2^8;%signal period

N_repeats=1000;%number of repeats at each point
...
E_zzb=zeros(N_SNR,N_fc);%initialize ziv-zakai bound
